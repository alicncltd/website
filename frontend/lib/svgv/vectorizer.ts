...
export function vectorizeFrame(
  ...
  const rdpTolerance = options.rdpTolerance ?? 2.5;
  const minPathLength = options.minPathLength ?? 12;
  const rasterPatchRatio = options.rasterPatchRatio ?? 0.15;
  const meshGridSize = options.meshGridSize ?? 48;
  ...
  // 1. Generate Global Background Mesh Gradient
        ...
        if (ratio >= rasterPatchRatio) {
          ...
          for (let y = by; y < by + bh; y++) {
            ...
            for (let x = bx; x < bx + bw; x++) {
              ...
              rgbData[dstIdx++] = rgbaPixels[srcIdx];
              rgbData[dstIdx++] = rgbaPixels[srcIdx + 1];
              rgbData[dstIdx++] = rgbaPixels[srcIdx + 2];
              visited[y * width + x] = 1;
            }
          }
...
function simplifyPath(points: { x: number; y: number }[], epsilon: number): { x: number; y: number }[] {
...
function getAverageColor(rgba: Uint8Array, width: number, height: number, cx: number, cy: number, radius = 1) {
